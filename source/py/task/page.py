...
from source.py.utils import joinPaths, read_json, read_text, run, write_json, write_text
...
def register_parser(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]):
    ...
    parser.add_argument("--sync", action="store_true", help="Sync latest page data and commit")
    return parser
...
def update_page(
    ...
    print("Update script")
    script_content = read_text(joinPaths("source", "py", "in_browser.py"))
    write_text(
    ...
    if woff2:
        ...
        run("python build.py --ttf-only --no-nerd-font --least-styles")
        run(f"ftcli converter ft2wf -f woff2 {var_dir}")
