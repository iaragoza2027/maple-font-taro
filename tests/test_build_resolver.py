...
from source.py.build.cli import parse_args
...
class BuildRuntimeContextFontPatcherTest(unittest.TestCase):
    ...
    def test_ensure_font_patcher_available_raises_for_missing_patcher_assets(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            ...
            with patch("source.py.build.resolver.check_font_patcher", return_value=False):
